        ...
        if (command.userPermissions) {
            ...
            if (!isIgnored) {
                if (typeof command.userPermissions === 'function') {
                    ...
                    const missing = message.member.voice.channelID ? message.member.voice.channel.permissionsFor(this.client.user).missing(command.userPermissions) : message.channel.permissionsFor(this.client.user).missing(command.userPermissions);
                    if (missing.length) {
                        this.emit(CommandHandlerEvents.MISSING_PERMISSIONS, message, command, 'user', missing);
                        return true;
                    }
